...
fn test_sr_latch() {
    ...
    assert!(
        q_init != q_bar_init,
        "Initial state should be complementary"
    );
...
fn test_compilation_and_nesting() {
    ...
    let in_a = sim.add_gate(GateType::Input);
    let in_b = sim.add_gate(GateType::Input);
    let xor_interface = sim
        .instantiate_chip(2, &library)
        .expect("Failed to compile XOR");
    let out = sim.add_gate(GateType::Output);
    ...
    let b_interface = sim2
        .instantiate_chip(4, &library)
        .expect("Failed to compile BufferTest");
...
fn test_serialization() {
    ...
    let deserialized: ChipBlueprint =
        serde_json::from_str(&serialized).expect("Failed to deserialize blueprint");
