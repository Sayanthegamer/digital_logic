...
impl Editor {
    pub fn handle_canvas_left_release(&mut self, mouse_pos_world: Vec2) {
        ...
        {
            let comp_type = self.get_component(comp_id).map(|c| c.comp_type);
            ...
            if box_w > 5.0 || box_h > 5.0 {
                ...
                for conn in &self.circuit.connections {
                    ...
                    if let (Some(&src), Some(&tgt)) = (src_comp_opt, tgt_comp_opt) {
                        ...
                        let wire_rect = get_wire_bounds(src_pos, tgt_pos, offset, conn.tgt_port);
        ...
        // End drag
        let was_dragging_comp = self.canvas.dragging_comp_id.is_some();
